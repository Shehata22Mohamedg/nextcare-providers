...
export default function ProviderDetails({ provider, onClose }: ProviderDetailsProps) {
  ...
  const fields = [
    ...
    { label: t("phone"), value: provider.phone, icon: Phone },
  ...
  return (
    <div className="fixed inset-0 bg-foreground/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-card rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] overflow-y-auto border border-border"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-5 border-b border-border bg-primary/5 rounded-t-2xl">
          <h2 className="font-bold text-lg text-foreground">{t("providerDetails")}</h2>
